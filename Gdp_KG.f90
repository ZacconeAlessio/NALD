...
implicit none
  ...
  real(8), parameter :: G_A = 80   ! Depends on temperature
...
write(F6,'(a,I1,a,f4.2,a)')'run',tej,'/G_dprime_T',T,'.data'
...
f=(nu*w)*gamma(i)*((swp(i)-w*w)**(+2) + nu*nu*w*w)**(-1)
sum=sum+f
end do
write(13,*)w, sum*constant
